...
def create_fc_layer(input, w, b, P=None, apply_relu=True, OWN=False):
    """
...
def grad_similarity_check(sess, model, images_for_grads, labels_for_grads, proj_matrices, task, task_labels=None, nd_logit_mask=None):
    """
    Compute the gradient similarity of current task with previous ones at different layers
    """
    curr_x = images_for_grads[task]
    curr_y = labels_for_grads[task]
    ind = np.random.choice(curr_x.shape[0], 100, replace=False)
    np.random.shuffle(ind)
    feed_dict = {model.x: curr_x[ind], model.y_: curr_y[ind], model.keep_prob: 1.0, 
                 model.train_phase: True, model.subspace_proj: proj_matrices[task]}
    if task_labels is None:
        feed_dict[output_mask] = np.ones(10)
    else:
        nd_logit_mask[:] = 0
    ...
    curr_grad_vars = sess.run(model.reg_gradients_vars, feed_dict=feed_dict)
    for t in range(task):
        t_x = images_for_grads[t]
        t_y = labels_for_grads[t]
        feed_dict = {model.x: t_x[ind], model.y_: t_y[ind], model.keep_prob: 1.0, 
                     model.output_mask: np.ones(10), model.train_phase: True, model.subspace_proj: proj_matrices[t]}
        if task_labels is None:
            feed_dict[output_mask] = np.ones(10)
        else:
            nd_logit_mask[:] = 0
            nd_logit_mask[t][task_labels[t]] = 1.0
            logit_mask_dict = {m_t: i_t for (m_t, i_t) in zip(model.output_mask, nd_logit_mask)}
            feed_dict.update(logit_mask_dict)
            feed_dict[model.mem_batch_size] = residual
        train_vars, t_grad_vars = sess.run([model.trainable_vars, model.reg_gradients_vars], feed_dict=feed_dict)
        print('\n\n\t\tTask:{}'.format(t))
        for v in range(len(train_vars)):
            if v % 2 == 0:
                print('\n\n\t\tLayer:{}'.format(v // 2))
            c_grad_l = np.reshape(curr_grad_vars[v][0], [-1])
            t_grad_l = np.reshape(t_grad_vars[v][0], [-1])
            cos_sim = np.inner(c_grad_l, t_grad_l)/ (np.linalg.norm(c_grad_l)*np.linalg.norm(t_grad_l) + 1e-15)
            print(cos_sim)

def grad_check(curr_grads, prev_task_grads):
    for tt, gg_tt in enumerate(prev_task_grads):
        print('\n\n\t\tTask:{}'.format(tt))
        for v in range(len(gg_tt)):
            if v % 2 == 0:
                print('Layer:{}'.format(v // 2))
            t_grad_l = np.reshape(curr_grads[v][0], [-1])
            tt_grad_l = np.reshape(gg_tt[v], [-1])
            cos_sim = np.inner(t_grad_l, tt_grad_l)/ (np.linalg.norm(t_grad_l) * np.linalg.norm(tt_grad_l) + 1e-15)
            print(cos_sim)
