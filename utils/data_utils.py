...
IMG_MEAN = np.array((0.406, 0.456, 0.485), dtype=np.float32) # -> images between 0 and 1
...
def _rotate_mnist(d, rotation):
    # result = np.zeros([d.shape[0], d.shape[1]])
    for i in range(d.shape[0]):
        image = d[i].reshape((28, 28)).astype(np.float32)
        img = Image.fromarray(image, mode='F')
        d[i] = np.array(img.rotate(rotation)).flatten().astype(np.float32)

    return d


def construct_rotate_mnist(num_tasks, min_angle=0, max_angle=180):
    """
    Construct a dataset for rotate mnist images

    Args:
        num_tasks   Number of tasks

    Returns:
        datasete    A rotated mnist dataset
    """
    # Download and store mnist dataset
    mnist = input_data.read_data_sets('MNIST_data', one_hot=True)
    ...
    for tt in range(num_tasks):
        ...
        min_rot = 1.0 * tt / num_tasks * (max_angle - min_angle) + min_angle
        max_rot = 1.0 * (tt + 1) / num_tasks * (max_angle - min_angle) + min_angle
        task_rotation = random.random() * (max_rot - min_rot) + min_rot
        print('task: {}, Rotation:{}'.format(tt, task_rotation))
        copied_mnist = deepcopy(mnist)
        sets = ["train", "validation", "test"]
        for set_name in sets:
            this_set = getattr(copied_mnist, set_name) # shallow copy
            this_set._images = _rotate_mnist(this_set.images, task_rotation)
            if set_name == "train":
                train = {
                    'images':this_set._images,
                    'labels':this_set.labels,
...
###### ImageNet Utils #############################
###################################################
def construct_split_miniImagenet(task_labels, data_file):
    """
    ...
    for task in task_labels:
        ...
        for cls in task:
            ...
            global_class_indices = np.column_stack(np.nonzero(imagenet_labels))
            ...
            # Each class has 600 images. Use 500 for training and 100 for test
            if count == 0:
            ...
            count += 1
        # Add the train/ test data in dict
        train = {
                'images':deepcopy(x_train),
                'labels':deepcopy(y_train),
                }
        test = {
                'images':deepcopy(x_test),
                'labels':deepcopy(y_test),
                }
        imagenet = {
            'train': train,
            'test': test,
