...
from utils_svd import svb

def _conv(x, kernel_size, out_channels, stride, var_list, orthog_init=False, pad="SAME", name="conv"):
    """
    ...
    with tf.variable_scope(name):
        ...
        if orthog_init:
            param_initializer=tf.initializers.orthogonal()
        else:
            param_initializer=tf.random_uniform_initializer(-stdv, stdv)
        w = tf.get_variable('kernel', [kernel_size, kernel_size, in_channels, out_channels],
                           tf.float32, initializer=param_initializer)

        # Append the variable to the trainable variables list
        var_list.append(w)

    # Bound the singular values
    #w = tf.stop_gradient(svb(w, [x_shape[1], x_shape[2]]))
    # Do the convolution operation
    output = tf.nn.conv2d(x, w, [1, stride, stride, 1], padding=pad)
    return output

def _fc(x, out_dim, var_list, name="fc", orthog_init=False):
    """
...
def _residual_block(x, trainable_vars, train_phase, apply_relu=True, orthog_init=False, name="unit"):
    """
    ...
    with tf.variable_scope(name) as scope:
        shortcut = x
        x = _conv(x, 3, in_channels, 1, trainable_vars, orthog_init=orthog_init, name='conv_1')
        x = _bn(x, trainable_vars, train_phase, name="bn_1")
        x = tf.nn.relu(x)
        x = _conv(x, 3, in_channels, 1, trainable_vars, orthog_init=orthog_init, name='conv_2')
...
def _residual_block_first(x, out_channels, strides, trainable_vars, train_phase, apply_relu=True, orthog_init=False, name="unit", is_ATT_DATASET=False):
    """
    ...
    with tf.variable_scope(name) as scope:
        ...
        if in_channels == out_channels:
            ...
            shortcut = _conv(x, 1, out_channels, strides, trainable_vars, orthog_init=orthog_init, name="shortcut")
            if not is_ATT_DATASET:
                shortcut = _bn(shortcut, trainable_vars, train_phase, name="bn_0")

        # Residual block
        x = _conv(x, 3, out_channels, strides, trainable_vars, orthog_init=orthog_init, name="conv_1")
        x = _bn(x, trainable_vars, train_phase, name="bn_1")
        x = tf.nn.relu(x)
        x = _conv(x, 3, out_channels, 1, trainable_vars, orthog_init=orthog_init, name="conv_2")
        x = _bn(x, trainable_vars, train_phase, name="bn_2")
