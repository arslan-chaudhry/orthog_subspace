...
"""
Training script for rotate MNIST experiment.
"""
...
from utils.data_utils import construct_rotate_mnist
...
## Model options
MODELS = ['VAN', 'PI', 'EWC', 'MAS', 'RWALK', 'A-GEM', 'S-GEM', 'FTR_EXT', 'PNN', 'ER-Reservoir', 'ER-Ringbuffer', 'SUBSPACE-PROJ', 'ER-SUBSPACE', 'PROJ-ANCHOR', 'PROJ-SUBSPACE-GP', 'MER'] #List of valid models 
IMP_METHOD = 'EWC'
SYNAP_STGTH = 75000
...
def train_task_sequence(model, sess, args):
    ...
    for runid in range(args.num_runs):
        ...
        for task in range(len(datasets)):
            ...
            for iters in range(num_iters):
                ...
                if args.train_single_epoch and not args.cross_validate_mode:
                    if (iters < 10) or (iters < 100 and iters % 10 == 0) or (iters % 100 == 0):
                        ...
                        fbatch = test_task_sequence(model, sess, datasets, args.online_cross_val, proj_matrices)
                        ftask.append(fbatch)
                ...
                if model.imp_method == 'VAN':
                    ...
                    mer_episodic_x_batch, mer_episodic_y_batch = episodic_images[er_mem_indices], episodic_labels[er_mem_indices]
                    sess.run(model.store_theta_i_not_w)
                    for mer_x, mer_y in zip(mer_episodic_x_batch, mer_episodic_y_batch):
                    ...
                    feed_dict = {model.x: train_x[offset:offset+residual], model.y_: train_y[offset:offset+residual],
                                model.training_iters: num_iters, model.train_step: iters, model.keep_prob: 1.0,
                                model.output_mask: logit_mask, model.train_phase: True, model.learning_rate: MER_S*args.learning_rate}
                    _, loss = sess.run([model.train, model.reg_loss], feed_dict=feed_dict)
                    sess.run(model.with_in_batch_reptile_update, feed_dict={model.mer_beta: MER_GAMMA})
            ...
            if args.train_single_epoch and not args.cross_validate_mode: 
                fbatch = test_task_sequence(model, sess, datasets, False, proj_matrices)
                ftask.append(fbatch)
                ...
                if MEASURE_PERF_ON_EPS_MEMORY:
                    ...
                    ftask = test_task_sequence(model, sess, eps_mem, args.online_cross_val, proj_matrices)
                else:
                    # List to store accuracy for all the tasks for the current trained model
                    ftask = test_task_sequence(model, sess, datasets, args.online_cross_val, proj_matrices)
...
def test_task_sequence(model, sess, test_data, cross_validate_mode, projection_matrices=None):
    """
    ...
    for task, _ in enumerate(test_data):
        ...
        if model.imp_method == 'PNN':
            ...
            feed_dict = {model.x: test_data[task]['test']['images'], 
                    ...
                    model.output_mask: logit_mask, model.train_phase: False}
            if model.imp_method in {'SUBSPACE-PROJ', 'ER-SUBSPACE', 'PROJ-ANCHOR', 'PROJ-SUBSPACE-GP'}:
                feed_dict[model.subspace_proj] = projection_matrices[task]
            acc = model.accuracy.eval(feed_dict = feed_dict)
...
def main():
    ...
    exper_meta_data = {'DATASET': 'ROTATE_MNIST',
    ...
    experiment_id = "ROTATE_MNIST_META_%s_%s_%r_%s-"%(args.imp_method, str(args.synap_stgth).replace('.', '_'),
    ...
    if args.cross_validate_mode:
        ...
        cross_validate_dump_file = args.log_dir + '/' + 'ROTATE_MNIST_%s_%s'%(args.imp_method, args.optim) + '.txt'
        with open(cross_validate_dump_file, 'a') as f:
            if MULTI_TASK:
                ...
                f.write('ORTHO:{} \t NUM_TASKS: {} \t EXAMPLES_PER_TASK: {} \t MEM_SIZE: {} \t ARCH: {} \t LR:{} \t LAMBDA: {} \t ACC: {} (+-{})\t Fgt: {} (+-{})\t QR: {}\t Time: {}\n'.format(args.maintain_orthogonality, args.num_tasks, args.examples_per_task, args.mem_size, args.arch, args.learning_rate, 
