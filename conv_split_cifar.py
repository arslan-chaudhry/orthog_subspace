...
def train_task_sequence(model, sess, datasets, args):
    ...
    for runid in range(args.num_runs):
        ...
        for task in range(len(task_labels)):
            ...
            for iters in range(num_iters):
                ...
                if model.imp_method == 'VAN':
                    ...
                    for tt in range(task+1):
                        if any(c_lab == t_lab for t_lab in task_labels[tt] for c_lab in labels_in_the_batch):
                            logit_mask[task_labels[tt]] = 1.0
                    if iters % 100 == 0:
                        print('Labels in batch:{}, logit mask:{}'.format(labels_in_the_batch, np.unique(np.nonzero(logit_mask))))
                    ...
                    if args.maintain_orthogonality:
                        if SVB:
                            ...
                            _, task_grads, kernel_isometry, loss = sess.run([model.accum_er_subspace_grads, model.kernel_grads, model.kernel_isometry, model.reg_loss], feed_dict=feed_dict)
                    ...
                    if STORE_GRAD_INP and task > 0:
                        batch_inp = []
                        for g_t, g_m in zip(task_grads, mem_grads):
                            g_t = np.reshape(g_t, [-1])
                            g_m = np.reshape(g_m, [-1])
                            u_g_t = g_t/ np.linalg.norm(g_t)
                            u_g_m = g_m/ np.linalg.norm(g_m)
                            batch_inp.append(np.dot(u_g_t, u_g_m))
                        grad_inp.append(batch_inp)
                    # Put the batch in the FIFO ring buffer
                    update_fifo_buffer(train_x[offset:offset+residual], train_y[offset:offset+residual], episodic_images, episodic_labels,
                                       task_labels[task], args.mem_size, count_cls, episodic_filled_counter)
                ...
                if (iters % 100 == 0):
                    ...
                    #print('Step {:d}\t Reg: {:.9f}\t TL: {:.3f}'.format(iters, kernel_isometry, loss))
            ...
            if STORE_GRAD_INP and task > 0:
                final_grad_inp.append(grad_inp)
        ...
        if STORE_GRAD_INP:
            grads_snapshot_file = args.log_dir + '/run_%d_ortho_%r'%(runid, args.maintain_orthogonality) + datetime.datetime.now().strftime("%y-%m-%d-%H-%M") + '.pickle'
            with open(grads_snapshot_file, 'wb') as f:
                pickle.dump(final_grad_inp, f)
...
def main():
    ...
    experiment_id = "SPLIT_CIFAR_META_%s_%s_%r_%s-"%(args.imp_method, str(args.synap_stgth).replace('.', '_'),
